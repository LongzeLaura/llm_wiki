...
| Static-source `current-scene/` pollution | Prompt/schema boundary plus writer-side explicit marker gate | `current-scene/` writes now require source text with `[RPG-LIVE]`; static lore, ending summaries, encyclopedia-like inputs, and unmarked live-looking text are rejected before write | Some supporting diagnostics remain lightweight heuristics, but `current-scene` write access is no longer opened by heuristic live-session wording |
...
- write-time rejection of `current-scene/` writes unless the source contains `[RPG-LIVE]`
