...
describe("RPG World Tick contract", () => {
  it("builds WorldTickInput directly from fixture wiki files and pre-action snapshot", async () => {
    ...
    expect(serialized).not.toContain("UNRELATED_GRAIL_EVENT_POISON")
    expect(serialized).not.toContain("UNRELATED_SIDE_QUEST_POISON")
...
async function writeWorldTickFixture(projectPath: string): Promise<void> {
  ...
  await writeFileRaw(`${projectPath}/wiki/events/grail-war.md`, "# Grail War\n\nUNRELATED_GRAIL_EVENT_POISON")
  ...
  await writeFileRaw(`${projectPath}/wiki/quests/distant-castle.md`, "# Distant Castle\n\nUNRELATED_SIDE_QUEST_POISON")
