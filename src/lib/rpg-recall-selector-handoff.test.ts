...
describe("RPG Recall Selector deterministic handoff", () => {
  it("builds RecallSelectorInput from post-action turn state plus fixture wiki readers", async () => {
    ...
    expect(paths).not.toContain("wiki/characters/runtime/saber.md")
    expect(paths).not.toContain("wiki/items/runtime/distant-relic.md")
    expect(serialized).not.toContain("UNRELATED_RECALL_RUNTIME_POISON")
...
async function writeRecallFixture(projectPath: string): Promise<void> {
  ...
  await writeFileRaw(`${projectPath}/wiki/characters/runtime/saber.md`, "# Saber Runtime\n\nUNRELATED_RECALL_RUNTIME_POISON")
  ...
  await writeFileRaw(`${projectPath}/wiki/items/runtime/distant-relic.md`, "# Distant Relic Runtime\n\nUNRELATED_RECALL_RUNTIME_POISON")
